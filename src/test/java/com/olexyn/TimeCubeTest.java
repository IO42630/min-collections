...
public class TimeCubeTest {
    ...
    private  static final List<Pair<Instant, String>> MANY_KEYS = new ArrayList<>();

    @BeforeClass
    public static void setUp() {

        for (int i = 0; i < 480000; i++) {
            MANY_KEYS.add(new Pair<>(Instant.now().minusSeconds(i), "value " + i));
        }
    ...
    public void rangeTest() {
        ...
        assertEquals(timeCube.get(now).getB(), "NOW");

        var inFiveYears = Instant.now().plus(Duration.ofDays(365 * 5));
        timeCube.put(inFiveYears, "inFiveYears");
        assertEquals(timeCube.get(inFiveYears).getB(), "inFiveYears");
    ...
    public void putPerformanceTest() {
            ...
            for ( var pair : MANY_KEYS ) {
                timeCube.put(pair.getA(), pair.getB());
            }
            ...
            start = Instant.now();
            for ( var pair : MANY_KEYS ) {
                treeMap.put(pair.getA(), pair.getB());
            }
            end = Instant.now();
            ...
            for ( var i = 0; i < putLoops; i++ ) {
                for ( var pair : MANY_KEYS ) {
                    assertEquals(timeCube.get(pair.getA()).getB(), pair.getB());
                }
            }
            end = Instant.now();
            ...
            for ( var i = 0; i < putLoops; i++ ) {
                for ( var pair : MANY_KEYS ) {
                    assertEquals(treeMap.get(pair.getA()), pair.getB());
                }
            }
            end = Instant.now();
